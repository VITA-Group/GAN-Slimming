...
parser.add_argument('-s', '--save_txt_dir', type=str)
...
def calculate_fid_given_paths(paths, batch_size=1, cuda=True, dims=2048):
...
def save_training_set_stat(path):
    dims = 2048
    block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
...
if __name__ == '__main__':
    ...
    fid_value = calculate_fid_given_paths(args.path,
                                          ...
                                          args.gpu != '',
                                          args.dims)
    if not os.path.exists(os.path.join(args.save_txt_dir)):
        os.makedirs(os.path.join(args.save_txt_dir))
    import re
    # print(args.path, type(args.path))
    found = re.search('epoch(.+?)_netG', args.path[0])
    if found:
        model_id = int(found.group(1))
        with open(os.path.join(args.save_txt_dir, 'fid_results_{}.txt'.format(args.indicator)), 'a+') as f:
            f.write('epoch{}: {}\n'.format(model_id, fid_value))
    else:
        with open(os.path.join('fid', args.datasets, 'fid_results_{}.txt'.format(args.indicator)), 'a+') as f:
            f.write('{}\n'.format(fid_value))
        print(fid_value)
    # print('FID: ', fid_value)
# model_id = int(re.search('epoch(.+?)_netG', args.path[0]).group(1))
#     with open(os.path.join(args.save_txt_dir, 'fid_results_{}.txt'.format(args.indicator)), 'a+') as f:
#         f.write('epoch{}: {}\n'.format(model_id, fid_value))